...
/**
 * The Class SABMB2BCustomersBulkWelcomeEmailAction.
 */
public class SABMB2BCustomersBulkWelcomeEmailAction extends AbstractListViewAction
{

	/** The Constant ICON_FUNC_APPROVAL_ACTION_AVAILABLE. */
	private static final String ICON_FUNC_APPROVAL_ACTION_AVAILABLE = "cockpit/images/email.png";

	/** The Constant LOG. */
	private static final Logger LOG = LoggerFactory.getLogger(SABMB2BCustomersBulkWelcomeEmailAction.class);
	...
	public EventListener getMultiSelectEventListener(final Context context)
	{
		...
		if (CollectionUtils.isNotEmpty(selectedItems))
		{
			ret = new EventListener()
			{
				...
				{
					final List<B2BCustomerModel> customers = changeWelcomeEmailFlags(selectedItems);
					final DefaultWizardContext hpc = new DefaultWizardContext();

					hpc.setAttribute("customers", customers);
					Wizard.show("welcomeEmailWizard", hpc);
	...
	/**
	 * Change welcome email flags.
	 *
	 * @param selectedItems the selected items
	 * @return the list
	 */
	private List<B2BCustomerModel> changeWelcomeEmailFlags(final List<TypedObject> selectedItems)
	{
		...
		for (final Iterator<TypedObject> iter = selectedItems.iterator(); iter.hasNext();)
		{
			...
			if (obj.getObject() instanceof B2BCustomerModel)
			{
				...
				customer.setOnboardWithWelcomeEmail(true);
				customer.setWelcomeEmailStatus(false);
