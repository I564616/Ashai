 ...
 *
 */
public class ALBActiveB2BTranslator extends SABMAbstracB2BCustomerTranslator
{
	private static final Logger LOG = LoggerFactory.getLogger(ALBActiveB2BTranslator.class);
	...
	public String performExport(final Item item) throws ImpExException
	{
			...
			for (final PrincipalGroupModel group : getCustomer().getGroups())
			{
				...
				if (group.getClass().equals(AsahiB2BUnitModel.class))
				{
					...
					if (!userDisabled && StringUtils.isNotEmpty(businessUnit.getCompanyCode())
							&& businessUnit.getCompanyCode().equals("sga"))
					{
		...
		LOG.debug("Result of ALB Active B2BTranslator translation for item [{}] is [{}]", item, result);
