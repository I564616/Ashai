 ...
 *
 */
public class APBActiveB2BTranslator extends SABMAbstracB2BCustomerTranslator
{

	private static final Logger LOG = LoggerFactory.getLogger(APBActiveB2BTranslator.class);
	...
	public String performExport(final Item item) throws ImpExException
	{
			...
			for (final PrincipalGroupModel group : getCustomer().getGroups())
			{
				...
				if (group.getClass().equals(AsahiB2BUnitModel.class))
				{
					...
					if (!userDisabled && StringUtils.isNotEmpty(businessUnit.getCompanyCode())
							&& businessUnit.getCompanyCode().equals("APB"))
					{
		...
		LOG.debug("Result of APB Active B2BTranslator translation for item [{}] is [{}]", item, result);
