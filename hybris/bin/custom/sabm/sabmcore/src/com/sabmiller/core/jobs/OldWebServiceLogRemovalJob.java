...
import com.sabm.core.webservicelog.service.SabmWebServiceLogService;
import com.sabmiller.core.model.OldWebServiceLogRemovalCronJobModel;
import com.sabmiller.integration.model.WebServiceLogModel;


/**
 * A Cron Job to clean up old WebServiceLog.
 *
 */
public class OldWebServiceLogRemovalJob extends AbstractJobPerformable<OldWebServiceLogRemovalCronJobModel>
{
	private static final Logger LOG = LoggerFactory.getLogger(OldWebServiceLogRemovalJob.class);

	@Resource(name = "timeService")
	private TimeService timeService;

	@Resource(name = "sabmWebServiceLogService")
	private SabmWebServiceLogService sabmWebServiceLogService;
	...
	@Override
	public PerformResult perform(final OldWebServiceLogRemovalCronJobModel job)
	{
		boolean status = true;
		...
		try
		{
			...
			List<WebServiceLogModel> webServiceLogs;

			//Remove old WebServiceLog data in batch considering the risk of returning a very large number of search results
			do
			{
				webServiceLogs = sabmWebServiceLogService
						.findOldWebServiceLog(new DateTime(timeService.getCurrentTime()).minusDays(age).toDate(), batchSize);
				modelService.removeAll(webServiceLogs);

			}
			while (webServiceLogs != null && !webServiceLogs.isEmpty());
			return new PerformResult(CronJobResult.SUCCESS, CronJobStatus.FINISHED);
		}
		catch (final Exception e)
		{
			status = false;
			LOG.error("Exception occurred during During webservice log cleanup", e);
			return new PerformResult(CronJobResult.ERROR, CronJobStatus.ABORTED);
		}
		finally
		{
