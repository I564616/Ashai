...
/**
 * A Cron Job to clean up old RepDrivenDealConditionStatus.
 *
 */
public class OldRepDrivenDealConditionStatusRemovalJob
		extends AbstractJobPerformable<OldRepDrivenDealConditionStatusRemovalCronJobModel>
{
	private static final Logger LOG = LoggerFactory.getLogger(OldRepDrivenDealConditionStatusRemovalJob.class);
	 ...
	 */
	@Override
	public PerformResult perform(final OldRepDrivenDealConditionStatusRemovalCronJobModel job)
	{
		boolean status = true;
		...
		try
		{
			...
			List<RepDrivenDealConditionStatusModel> repDrivenDealConditionStatus;

			//Remove old repDrivenDealConditionStatus data in batch considering the risk of returning a very large number of search results
			do
			{
				repDrivenDealConditionStatus = repDrivenDealConditionStatusService.findRepDrivenDealConditionStatus(
						new DateTime(timeService.getCurrentTime()).minusDays(age).toDate(), batchSize);
				modelService.removeAll(repDrivenDealConditionStatus);

			}
			while (repDrivenDealConditionStatus != null && !repDrivenDealConditionStatus.isEmpty());
			return new PerformResult(CronJobResult.SUCCESS, CronJobStatus.FINISHED);
		}
		catch (final Exception e)
		{
			status = false;
			LOG.error("Exception occurred during Old rep DrivenDeal conditionstatus cleanup", e);
			return new PerformResult(CronJobResult.ERROR, CronJobStatus.ABORTED);
		}
		finally
		{
