...
public class ProductVisibleFlagHandler implements DynamicAttributeHandler<Boolean, ProductModel>
{
	private SabmProductService productService;

	/**
	 * Return if product is visible on site
	 *
	 * @param product
	 *           the product
	 * @return true if product is visible on site
	 */
	@Override
	public Boolean get(final ProductModel product)
	{
		return product instanceof SABMAlcoholVariantProductEANModel
		    && productService.isProductVisible((SABMAlcoholVariantProductEANModel) product);
	}
	...
	{
		throw new UnsupportedOperationException("Set of dynamic attribute 'visible' of Product is disabled!");
	}
