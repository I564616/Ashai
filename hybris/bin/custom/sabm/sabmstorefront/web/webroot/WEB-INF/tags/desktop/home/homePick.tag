...
<div class="row product-pick addToCartEventTag">
    <div class="col-xs-12  <c:if test="${!isProductPackTypeAllowed}"> disabled-productPackTypeNotAllowed</c:if>">
    ...
    <div class="col-xs-12">
		...
		<form:form id="cubPicksAddToCartForm${count}" action="${addToCartUrl}" method="post" class="add_to_cart_form">
			...
			<c:choose>
                ...
                <c:otherwise>
			  	  <button type="${buttonType}" class="btn btn-primary btn-block bde-view-only <c:if test="${stockLevelStatusCode eq 'outOfStock' || cubStockStatus eq 'outOfStock'}">out-of-stock</c:if>"
