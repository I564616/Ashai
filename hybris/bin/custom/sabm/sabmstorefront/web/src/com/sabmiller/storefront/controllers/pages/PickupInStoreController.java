...
import com.sabmiller.storefront.security.cookie.CustomerLocationCookieGenerator;
...
public class PickupInStoreController extends SabmAbstractSearchPageController
{
	...
	@PostMapping(PRODUCT_CODE_PATH_VARIABLE_PATTERN + "/pointOfServices")
			...
			@RequestParam(value = "qty", defaultValue = "0") final Long qty, final HttpServletResponse response, final Model model)
	{
		...
		return getPointOfServiceForStorePickup(productCode, XSSFilterUtil.filter(locationQuery), geoPoint, page, showMode, sortCode, cartPage,
				entryNumber, model, RequestMethod.POST, response);
	}

	@GetMapping(PRODUCT_CODE_PATH_VARIABLE_PATTERN + "/pointOfServices")
			...
			final HttpServletResponse response, final Model model)
	{
		...
		return getPointOfServiceForStorePickup(XSSFilterUtil.filter(productCode), location, geoPoint, page, showMode, sortCode, cartPage, entryNumber,
				model, RequestMethod.GET, response);
	}

	protected String getPointOfServiceForStorePickup(final String productCode, final String locationQuery, final GeoPoint geoPoint,
			final int page, final AbstractSearchPageController.ShowMode showMode, final String sortCode, final Boolean cartPage,
			final Long entryNumber, final Model model, final RequestMethod requestMethod, final HttpServletResponse response)
	{
