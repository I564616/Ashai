...
public class DeliveryAddressCheckoutStepController extends AbstractCheckoutStepController
{
	...
	private DefaultSabmB2BUnitFacade b2bCommerceUnitFacade;
			...
			final RedirectAttributes redirectAttributes) throws CMSItemNotFoundException
	{
		...
		{
			addressData = getCheckoutFacade().getDeliveryAddressForCode(XSSFilterUtil.filter(editAddressCode));
		}
			...
			final RedirectAttributes redirectAttributes)
	{
		...
		{
			final AddressData selectedAddressData = getCheckoutFacade().getDeliveryAddressForCode(XSSFilterUtil.filter(selectedAddressCode));
