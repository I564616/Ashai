...
import com.sabmiller.storefront.security.BruteForceAttackCounter;
...
public class DefaultBruteForceAttackCounter implements BruteForceAttackCounter
{
	...
	public void registerLoginFailure(final String userUid)
	{
		...
		{
			final LoginFailure count = get(prepareUserUid(userUid), 0);
	...
	{
		return StringUtils.isNotEmpty(userUid) && maxFailedLogins.compareTo(get(prepareUserUid(userUid), 0).getCounter()) <= 0;
	}
	...
	public int getUserFailedLogins(final String userUid)
	{
		if (StringUtils.isNotEmpty(userUid))
		{
			return get(prepareUserUid(userUid), 0).getCounter();
		}
		else
		{
			return 0;
