...
/**
 * Filter that initializes the session for the sabmstorefront. This is a spring configured filter that is executed by
...
public class StorefrontFilter extends OncePerRequestFilter
{
	...
	private static final Logger LOG = LoggerFactory.getLogger(StorefrontFilter.class);
			...
			final FilterChain filterChain) throws IOException, ServletException
	{
		...
		final String requestHost = StringUtils.substringBefore(request.getHeader(HOST), ":");
		final String hostServerName = StringUtils.substringBefore(request.getServerName(), ":");

		LOG.debug("HOST" + requestHost);
		LOG.debug("host server name" + hostServerName);

		if (!StringUtils.equalsIgnoreCase(requestHost, hostServerName))
		{
			LOG.error("the header host is not matching - Pen test fix");
			return;
		}
