...
import com.sabmiller.fulfilmentprocess.test.actions.TestActionTemp;
...
public class PaymentIntegrationTest extends ServicelayerTest
{
	...
	public static void prepare() throws Exception //NOPMD
	{
		...
		xmlReader.loadBeanDefinitions(new ClassPathResource(
				"/sabmfulfilmentprocess/test/sabmfulfilmentprocess-spring-test.xml"));
		xmlReader.loadBeanDefinitions(new ClassPathResource(
				"/sabmfulfilmentprocess/test/sabmfulfilmentprocess-spring-test-fraudcheck.xml"));
		xmlReader
				.loadBeanDefinitions(new ClassPathResource("/sabmfulfilmentprocess/test/process/order-process-spring.xml"));
		...
		definitonFactory.add("classpath:/sabmfulfilmentprocess/test/process/payment-process.xml");
	...
	public void setUpProductCatalogue()
	{
		try
		{
			...
			baseSiteService.setCurrentBaseSite(baseSiteService.getBaseSiteForUID("sabmStore"), false);
	...
	public static void removeProcessDefinitions()
	{
		...
		final XmlBeanDefinitionReader xmlReader = new XmlBeanDefinitionReader((BeanDefinitionRegistry) beanFactory);
		xmlReader.loadBeanDefinitions(new ClassPathResource(
				"/sabmfulfilmentprocess/test/sabmfulfilmentprocess-spring-testcleanup.xml"));
	...
	public void testPaymentIntegrationAccepted() throws InterruptedException, CalculationException
	{
		...
		businessProcessService.startProcess(orderProcess);
	...
	protected OrderModel placeTestOrder(final boolean valid) throws InvalidCartException, CalculationException
	{
		...
		deliveryAddress.setOwner(b2bCustomerModel);
		...
		paymentInfo.setUser(b2bCustomerModel);
	...
	protected OrderProcessModel createProcess(final String processName)
	{
		...
		return businessProcessService.createProcess(id, processName);
		//return processService.startProcess(, processName);
	}
