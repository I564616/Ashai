ACC.product = {
    ...
    _autoload: [
        ...
        "bindFacets",
		"apbBonusInitPageEvents",
		"hideExistingErrorMessages"
    ],
	...
	apbBonusInitPageEvents: function () {
			$(document).on("submit", '.add_bonus_to_cart_form', function (e) {
				e.preventDefault();
				ACC.product.hideExistingErrorMessages();
				
				var currentPage = $("#checkPage").val();
				var productCode = $(this).find(".bonus-stock-plp").prop("id");
				
				var input = $('.addToCartForm'+productCode).find('.qty.js-qty-selector-input');
				...
				$(".js-bonus-qty-input").val(inputVal);
				...
				if (overMax) {
					...
					var bonusFormData = $(this).serialize();
					$.ajax({
