ACC.cartitem = {
	...
	bindEditQuantityCartItemButtons: function ()
	{
        ...
        $(document).on("click", '.cart-total-update-js', function () {
			var productCode = $(this).find(".productQty").val();
...
checkQtySelector: function (self, mode) {
    ...
    updateQtyValue: function (self, value) {
        var input = $(self).parents(".js-cart-qty-selector").find(".js-cart-qty-selector-input");
		$(".cartqty_" + input.prop("id")).val(value);
        input.val(value);
        var form = $(input).closest('form');
    	var initialCartQuantity = form.find('input[name=initialQuantity]').val();
	...
	updateCartSections: function(data){
		 ...
		 if ($(".cart-bonus-update-js").length > 0) {				
			$(".cart-bonus-update-js").attr("style","opacity:0.5;");
			$(".cart-bonus-update-js").attr("disabled", "disabled");	
		 } 
		
		var onlyBonusProduct = $(data).find('#hasBonusStockProduct').val();
		if ((onlyBonusProduct != null) && (onlyBonusProduct === "true")) {
			//only bonus products in cart
			$("#saveCartButton").attr("disabled", "disabled");
			$(".js-continue-checkout-button").removeAttr("disabled"); //hence enable checkout
		}
	},
	
	checkIfMaxBonusProducts: function(){
		if ((sessionStorage.getItem("maxBonusProductsReached") != null) && (sessionStorage.getItem("maxBonusProductsReached") === "true")) {
			var maxReached = sessionStorage.getItem("maxBonusProductsReached");
			$('[name="updatedQtyMsg"]').val("Quantity updated to max allowed.");
			$('[name="isMessageTypeError"]').val("true");
		}
...
$(window).load(function () {
	...
	if ($('#hasBonusStockProduct').val() === 'true') { //enable checkout buttons if cart has only bonus products
