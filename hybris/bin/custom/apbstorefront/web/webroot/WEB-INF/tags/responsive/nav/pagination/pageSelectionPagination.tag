        ...
        <c:if test="${hasPreviousPage}">
            <li class="pagination-prev">
                ...
                <ycommerce:testId code="searchResults_previousPage_link">
                    <c:choose>
                        <c:when test="${request.getParameter('startDate') != null}">
                            <a href="${previousPageUrl}&startDate=${request.getParameter('startDate').replace('/','%2F')}&endDate=${request.getParameter('endDate').replace('/','%2F')}" rel="next" class="glyphicon glyphicon-chevron-left"></a>
                        </c:when>
                        <c:otherwise>
            ...
            <c:choose>
                <c:when test="${searchPageData.pagination.currentPage + 1 ne pageNumber}">
                    ...
                    <ycommerce:testId code="pageNumber_link">
                        <c:choose>
                            <c:when test="${request.getParameter('startDate') != null}">
                                <li><a class="${linkClass}" href="${pageNumberUrl}&startDate=${request.getParameter('startDate').replace('/','%2F')}&endDate=${request.getParameter('endDate').replace('/','%2F')}">${pageNumber}</a></li>
                            </c:when>
                            <c:otherwise>
        ...
        <c:if test="${hasNextPage}">
            <li class="pagination-next">
                ...
                <ycommerce:testId code="searchResults_nextPage_link">
                    <c:choose>
                        <c:when test="${request.getParameter('startDate') != null}">
                            <a href="${nextPageUrl}&startDate=${request.getParameter('startDate')}&endDate=${request.getParameter('endDate')}" rel="next" class="glyphicon glyphicon-chevron-right"></a>
                        </c:when>
                        <c:otherwise>
