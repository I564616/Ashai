...
<%@ attribute name="arrayList" required="false" type="java.util.ArrayList" %>
			...
			<div class="control">
				<form:select id="${escapedIdKey}" path="${escapedPath}" cssClass="${fn:escapeXml(selectCSSClass)}" tabindex="${fn:escapeXml(tabindex)}" disabled="${disabled}">
					...
					<c:choose>
					    ...
					    <c:otherwise>
					        <c:forEach items="${arrayList}" var="element">
                               <form:option value="${element.code}" label="${element.name}"/>
                           </c:forEach>
					    </c:otherwise>
					</c:choose>
