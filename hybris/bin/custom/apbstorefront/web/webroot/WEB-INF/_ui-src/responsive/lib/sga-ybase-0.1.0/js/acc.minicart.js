ACC.minicart = {
		        ...
		        mouseenter: function(e){
		        	if ($(window).width() >= 1280) {
						...
						var cartName = ($(this).find(".js-mini-cart-count").html() !== 0) ? $(this).data("miniCartName") : $(this).data("miniCartEmptyName");
							...
							/*To-Do-button ui change on disable*/
							$(document).on('click', '#mini-cart-checkout-btn', function(){
								var minOrderCheck = $(this).data("minOrderCheck");
								if(minOrderCheck){
									return false;
								} else {
									ACC.sgalogin.validateCheckoutCart();
								}
							});
					}
		        }
    ...
    openCartSummary: function () {
		...
		var onCreditBlock = sessionStorage.getItem("onCreditBlock");
		if(onCreditBlock === 'true'){
			var classToAdd = "minicart-colorbox disabled-checkout-btn";
		} else {
			var classToAdd = "minicart-colorbox";
		}
...
/*    if ($('.page-cartPage input[name=hasBonusStockProduct]').length !== 0) {
        if ($('.page-cartPage input[name=hasBonusStockProduct]').val() === 'true') {
