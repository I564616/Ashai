...
$('.button, .btn, button, .js-site-logo, .mini-cart-link').bind('click', function (event) {
    if (($('#checkPage').val() == 'checkoutPage' &&  $(this).parent().hasClass('js-site-logo')) || checkSpinnerApplicable(event, excludeClasses, excludeIds)) {
        applySpinner();
    }
});
...
function checkSpinnerApplicable(event, exClasses, exIds){
    var spinApply= true;
    if ($('#checkPage').val() == 'checkoutPage' || $('#checkPage').val() == 'paymentdetail' || $('#checkPage').val() == 'quickOrderPage' || $('#checkPage').val() == 'homepage') {
        spinApply = false;
    }
