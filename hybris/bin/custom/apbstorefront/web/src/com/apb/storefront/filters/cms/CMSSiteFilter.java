 ...
 */
package com.apb.storefront.filters.cms;
...
public class CMSSiteFilter extends OncePerRequestFilter implements CMSFilter
{
	...
	private static final Logger LOG = LoggerFactory.getLogger(CMSSiteFilter.class);
			...
			throws IOException
	{
		...
		else if (!(SiteChannel.B2B.equals(cmsSiteModel.getChannel()) || SiteChannel.B2C.equals(cmsSiteModel.getChannel()))) // Restrict to configured channel
		{
			// CMS site that we looked up was for an unsupported channel
			LOG.error("The channel of the site is"+cmsSiteModel.getChannel()+SiteChannel.B2B+SiteChannel.B2C);
	...
	protected CMSSiteModel getCurrentCmsSite()
	{
		...
		catch (final JaloObjectNoLongerValidException e)
		{
			...
			{
				LOG.debug("JaloObjectNoLongerValidException occurred...", e);
			}
			return null;
		}
	}
