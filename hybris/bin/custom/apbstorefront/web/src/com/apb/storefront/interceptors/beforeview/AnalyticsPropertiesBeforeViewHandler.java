 ...
 */
package com.apb.storefront.interceptors.beforeview;
...
public class AnalyticsPropertiesBeforeViewHandler implements BeforeViewHandler
{
	...
	private static final String TAG_MANAGER_ID = "google.tag.manager.tracking.id";
	private static final String TAG_MANAGER_PREFIX = "googleTagManagerId";
	...
	public void beforeView(final HttpServletRequest request, final HttpServletResponse response, final ModelAndView modelAndView)
	{
		...
		addHostProperty(serverName, modelAndView,
				ThirdPartyConstants.Google.ANALYTICS_TRACKING_ID + "." + cmsSiteService.getCurrentSite().getUid(),
				ANALYTICS_TRACKING_ID);
		// Add config properties for jirafe analytics
		
		addHostProperty(serverName, modelAndView, TAG_MANAGER_ID + "." + cmsSiteService.getCurrentSite().getUid(), 
				TAG_MANAGER_PREFIX);
		...
		if (jirafeMapCache.get(currSpecKey) != null
				&& org.apache.commons.lang3.StringUtils.isNotBlank(jirafeMapCache.get(currSpecKey).toString()))
		{
