...
import com.apb.storefront.web.wrappers.RemoveEncodingHttpServletRequestWrapper;
...
public class AcceleratorRememberMeServices extends TokenBasedRememberMeServices
{
	...
	private static final String j_rememberMe = "j_rememberMe";

	private static final Logger LOG = LoggerFactory.getLogger(AcceleratorRememberMeServices.class);
	...
	{
		/* Set cookie only if the j_remember me checkbox is enabled */
		if (Boolean.parseBoolean(request.getParameter(j_rememberMe)) && !getCheckoutCustomerStrategy().isAnonymousCheckout())
		{
			LOG.info("Setting cookie only when remember-checkbox is checked...");
