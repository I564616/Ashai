...
public class ProductPageController extends ApbAbstractPageController
{
	...
	private static final String ERROR_CMS_PAGE = "404";
			...
			throws CMSItemNotFoundException, UnsupportedEncodingException
	{
		...
		final ProductData productData = populateProductDetailForDisplay(productCode, model, request, extraOptions);
		if (productData.getLicenseRequired() != null && productData.getLicenseRequired() && !asahiSiteUtil.isSga())
		{
			if (checkSuperUser())
			{
		...
		{
			model.addAttribute("productRecommendation", asahiProductRefernceFacade.getPDPRecommendedProducts(productCode));			
		}
		if (asahiSiteUtil.isSga()) {
			model.addAttribute("dealsData", productData.getAsahiDealsInfo());
		}

		model.addAttribute("futureStockEnabled", Boolean.valueOf(Config.getBoolean(FUTURE_STOCK_ENABLED, false)));
		model.addAttribute("productNewStatusAvailable",this.asahiConfigurationService.getString("sga.product.status.available", "false"));
		model.addAttribute("priceError",
				userService.getCurrentUser().getUid().equalsIgnoreCase(ApbStoreFrontContants.ANONYMOUS) ? false
						: productData.getPriceError());
			...
			throws CMSItemNotFoundException
	{
		...
		if (result.hasErrors())
		{
			...
			if ((getPageForProduct(productCode)).getBackgroundImage() != null)
			{
				model.addAttribute("media", (getPageForProduct(productCode)).getBackgroundImage().getURL());
			}
			...
			final HttpServletRequest request, final List<ProductOption> extraOptions) throws CMSItemNotFoundException
	{
		...
		final List<ProductOption> options = new ArrayList<>(
				Arrays.asList(ProductOption.VARIANT_FIRST_VARIANT, ProductOption.BASIC, ProductOption.URL, ProductOption.SUMMARY,
		...
		sortVariantOptionData(productData);
		/**
		 * To check if the product is excluded or not
		 */
		if (asahiSiteUtil.isSga() && !userFacade.isAnonymousUser()
				&& (!(productData.getActive().booleanValue()) || !inclusionExclusionProductStrategy.isProductIncluded(productCode)))
		{
			GlobalMessages.addErrorMessage(model, ApbStoreFrontContants.PRODUCT_UNAVAILABLE_MESSAGE_TEXT);
			model.addAttribute("disablePdp", Boolean.TRUE);
		}
	...
	protected void populateProductData(final ProductData productData, final Model model)
	{
		...
		//Need to validate
		if (BooleanUtils.isTrue(productData.getConfigurable()))
		{
