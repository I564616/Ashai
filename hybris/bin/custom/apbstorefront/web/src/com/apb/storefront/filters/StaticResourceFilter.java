...
{
    private static final Logger LOGGER = Logger.getLogger(StaticResourceFilter.class);
    ...
    private AsahiAdhocCoreUtil asahiConfigJSUtil = null;
    ...
    {
        if(asahiConfigJSUtil == null){
            asahiConfigJSUtil = Registry.getApplicationContext().getBean(AsahiAdhocCoreUtil.class);
        }
        ...
        else if(asahiConfigJSUtil.getConfigValue("use.cache-control.config.from.db").equalsIgnoreCase("true")){
            final String key = HEADER_PROPERTIES_PREFIX + CACHE_CONTROL;
            final String configValFromDb = asahiConfigJSUtil.getConfigValue(key);
            if(headerParams.containsKey(CACHE_CONTROL) && !headerParams.get(CACHE_CONTROL).equalsIgnoreCase(configValFromDb)){
                headerParams.put(CACHE_CONTROL, configValFromDb);
            }
        }
    ...
    protected void addHeaderParamsFromConfig(final ConfigIntf config)
    {
        for (final String key : config.getAllParameters().keySet())
        {
            if (key.startsWith(HEADER_PROPERTIES_PREFIX))
            {
                ...
                if(asahiConfigJSUtil.getConfigValue("use.cache-control.config.from.db").equalsIgnoreCase("true") && !headerKey.isEmpty()){
                    headerParams.put(headerKey, asahiConfigJSUtil.getConfigValue(key));
                }
